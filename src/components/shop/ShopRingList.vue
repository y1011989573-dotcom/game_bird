<template>
	<div class="grid grid-cols-4 gap-2">
		...
		>
			...
			<div class="bg-pink-300 w-full aspect-square mb-2 flex items-center justify-center overflow-hidden">
				<img
					:src="getImageUrl('ring', item.nickname || 'default')"
					:alt="item.nickname"
			...
			<div class="grow text-sm p-2">
				...
				<div class="text-xs text-gray-600 mt-1 space-y-0.5">
					<div v-if="item.bird_time">
						配对时间: -{{ item.bird_time }}分钟
					</div>
					<div v-if="item.bird_weight_min || item.bird_weight_max">
						体重增幅: {{ item.bird_weight_min }}~{{ item.bird_weight_max }}g
					</div>
					<div v-if="item.need_heart" class="text-red-500">
						需要亲密度: {{ item.need_heart }}
					</div>
				</div>
			</div>
		</el-card>
	...
	<el-dialog v-model="dialogVisible" class="p-0!" width="90%" :show-close="false" header-class="p-0!">
		<el-image :src="getImageUrl('ring', currentItem?.nickname, '_big')" fit="cover" loading="lazy">
		...
		<div class="flex flex-col p-4! gap-2!">
			...
			<div class="mt-2 bg-pink-50 p-3 rounded">
				...
				<div class="space-y-1.5">
					<div v-if="currentItem?.bird_time" class="text-sm text-pink-800">
						⏰ 配对时间缩短: {{ currentItem.bird_time }} 分钟
					</div>
					<div v-if="currentItem?.bird_weight_min || currentItem?.bird_weight_max" class="text-sm text-pink-800">
						📊 幼鸟体重增幅: {{ currentItem.bird_weight_min }}~{{ currentItem.bird_weight_max }}g
					</div>
					<div v-if="currentItem?.need_heart" class="text-sm text-red-600 font-bold">
						❤️ 需要亲密度: {{ currentItem.need_heart }}
					</div>
				</div>
			</div>
