<template>
    <div class="grid grid-cols-4 gap-2">
        ...
        >
            <div class="bg-purple-300 w-full aspect-square mb-2 flex items-center justify-center overflow-hidden">
                <img
                    :src="getImageUrl('common', item.nickname || 'default')"
                    :alt="item.nickname"
    ...
    <el-dialog v-model="dialogVisible" class="p-0!" width="90%" :show-close="false" header-class="p-0!">
        <el-image :src="getImageUrl('common', currentItem?.nickname, '_big')" fit="cover" loading="lazy">
...
const buyItem = async () => {
    ...
    loading.value = true
    try {
        const res = await game.game_item_common.buy(currentItem.value.id, buyCount.value)
