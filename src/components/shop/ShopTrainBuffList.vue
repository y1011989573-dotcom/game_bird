<template>
	<div class="grid grid-cols-4 gap-2">
		...
		>
			...
			<div class="bg-blue-300 w-full aspect-square mb-2 flex items-center justify-center overflow-hidden">
				<img
					:src="getImageUrl('train_buff', item.nickname || 'default')"
					:alt="item.nickname"
	...
	<el-dialog v-model="dialogVisible" class="p-0!" width="90%" :show-close="false" header-class="p-0!">
		<el-image :src="getImageUrl('train_buff', currentItem?.nickname, '_big')" fit="cover" loading="lazy">
		...
		<div class="flex flex-col p-4! gap-2!">
			...
			<div class="mt-2">
				...
				<div class="bg-purple-50 p-2 rounded text-sm">
					训练时间加成: <span class="text-purple-600 font-bold">+{{ currentItem?.time || 0 }}%</span>
				</div>
			</div>
...
const buyItem = async () => {
	...
	loading.value = true
	try {
		const res = await game.game_item_train_buff.buy(currentItem.value.id, buyCount.value)
