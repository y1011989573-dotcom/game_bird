<template>
	<div class="grid grid-cols-4 gap-2">
		...
		>
			...
			<div class="bg-green-300 w-full aspect-square mb-2 flex items-center justify-center overflow-hidden">
				<img
					:src="getImageUrl('bird_exp', item.nickname || 'default')"
					:alt="item.nickname"
	...
	<el-dialog v-model="dialogVisible" class="p-0!" width="90%" :show-close="false" header-class="p-0!">
		<el-image :src="getImageUrl('bird_exp', currentItem?.nickname, '_big')" fit="cover" loading="lazy">
		...
		<div class="flex flex-col p-4! gap-2!">
			...
			<!-- 经验卡片特有：显示经验值 -->
			<div v-if="currentItem?.exp" class="mt-2 bg-blue-50 p-3 rounded">
...
const buyItem = async () => {
	...
	loading.value = true
	try {
		const res = await game.game_item_bird_exp.buy(currentItem.value.id, buyCount.value)
