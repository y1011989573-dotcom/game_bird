<template>
	<div class="grid grid-cols-5 gap-4">
		...
		>
			...
			<div class="bg-blue-300 w-full aspect-square mb-2 flex items-center justify-center overflow-hidden">
				<img
					:src="getImageUrl('bait', item.game_item_bait?.nickname || 'default')"
					:alt="item.game_item_bait?.nickname"
			...
			<div class="grow text-sm p-2 min-w-0">
				<p class="text-orange-500 font-bold truncate">
					{{ item.game_item_bait?.nickname || '未知物品' }}
				</p>
				...
				<!-- 显示可吸引的鸟类 -->
				<div v-if="item.game_item_bait?.game_item_bait_bird?.length" class="text-xs text-green-500 mt-1 truncate">
					可吸引: {{ item.game_item_bait.game_item_bait_bird.map(b => b.game_bird?.nickname).join(', ') }}
				</div>
			</div>
		</el-card>
	</div>
	...
	<el-dialog v-model="dialogVisible" class="p-0!" width="90%" :show-close="false" header-class="p-0!">
		<el-image :src="getImageUrl('bait', currentItem?.game_item_bait?.nickname, '_big')" fit="cover" loading="lazy">
		...
		<div class="flex flex-col p-4! gap-2!">
			<div class="mb-3 text-base">{{ currentItem?.game_item_bait?.nickname }}</div>
			<div class="text-gray-500 text-sm">{{ currentItem?.game_item_bait?.desc }}</div>
			...
			<!-- 饵料特有：显示可吸引的鸟类详细列表 -->
			<div v-if="currentItem?.game_item_bait?.game_item_bait_bird?.length" class="mt-2">
				<div class="text-sm font-bold text-green-600 mb-2">可吸引的鸟类：</div>
				<div class="grid grid-cols-2 gap-2">
					<div
						v-for="baitBird in currentItem.game_item_bait.game_item_bait_bird"
						:key="baitBird.id"
			...
			<div class="flex justify-between mt-4">
				...
				<div class="flex gap-4">
					...
					<el-button type="success" @click="useItem">下饵</el-button>
				</div>
			</div>
		</div>
	</el-dialog>
