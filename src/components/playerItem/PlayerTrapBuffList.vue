<template>
	<div class="grid grid-cols-5 gap-4">
		...
		>
			...
			<div class="bg-blue-300 w-full aspect-square mb-2 flex items-center justify-center overflow-hidden">
				<img
					:src="getImageUrl('trap_buff', item.game_item_trap_buff?.nickname || 'default')"
					:alt="item.game_item_trap_buff?.nickname"
			...
			<div class="grow text-sm p-2 min-w-0">
				<p class="text-orange-500 font-bold truncate">
					{{ item.game_item_trap_buff?.nickname || '未知物品' }}
				</p>
				...
				<div class="text-xs text-purple-500 mt-1">
					时间加成: +{{ item.game_item_trap_buff?.buff_time || 0 }}%
				</div>
			</div>
		</el-card>
	</div>
	...
	<el-dialog v-model="dialogVisible" class="p-0!" width="90%" :show-close="false" header-class="p-0!">
		<el-image :src="getImageUrl('trap_buff', currentItem?.game_item_trap_buff?.nickname, '_big')" fit="cover" loading="lazy">
		...
		<div class="flex flex-col p-4! gap-2!">
			<div class="mb-3 text-base">{{ currentItem?.game_item_trap_buff?.nickname }}</div>
			<div class="text-gray-500 text-sm">{{ currentItem?.game_item_trap_buff?.desc }}</div>
			...
			<div class="mt-2">
				...
				<div class="bg-purple-50 p-2 rounded text-sm">
					陷阱时间加成: <span class="text-purple-600 font-bold">+{{ currentItem?.game_item_trap_buff?.buff_time || 0 }}%</span>
				</div>
			</div>
			...
			<div class="flex justify-between mt-4">
				...
				<div class="flex gap-4">
					...
					<el-button type="success" @click="useItem" :loading="loading" :disabled="loading">使用</el-button>
				</div>
			</div>
		</div>
	</el-dialog>
...
const useItem = async () => {
	...
	loading.value = true
	try {
		const res = await game.player_item_trap_buff.use(currentItem.value.id, useCount.value)
