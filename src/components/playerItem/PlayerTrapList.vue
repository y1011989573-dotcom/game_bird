<template>
	<div class="grid grid-cols-5 gap-4">
		...
		>
			...
			<div class="bg-purple-300 w-full aspect-square mb-2 flex items-center justify-center overflow-hidden">
				<img
					:src="getImageUrl('trap', item.game_item_trap?.nickname || 'default')"
					:alt="item.game_item_trap?.nickname"
			...
			<div class="grow text-sm p-2 min-w-0">
				<p class="text-orange-500 font-bold truncate">
					{{ item.game_item_trap?.nickname || '未知物品' }}
				</p>
				...
				<!-- 显示可捕获的鸟类 -->
				<div v-if="item.game_item_trap?.game_bird" class="text-xs text-green-500 mt-1 truncate">
					可捕获: {{ item.game_item_trap.game_bird.nickname }}
				</div>
			</div>
		</el-card>
	</div>
	...
	<el-dialog v-model="dialogVisible" class="p-0!" width="90%" :show-close="false" header-class="p-0!">
		<el-image :src="getImageUrl('trap', currentItem?.game_item_trap?.nickname, '_big')" fit="cover" loading="lazy">
		...
		<div class="flex flex-col p-4! gap-2!">
			<div class="mb-3 text-base">{{ currentItem?.game_item_trap?.nickname }}</div>
			<div class="text-gray-500 text-sm">{{ currentItem?.game_item_trap?.desc }}</div>
			...
			<!-- 陷阱特有：显示可捕获的鸟类信息 -->
			<div v-if="currentItem?.game_item_trap?.game_bird" class="mt-2 bg-purple-50 p-3 rounded">
				<div class="text-sm font-bold text-purple-600 mb-1">专属捕获：</div>
				<div class="text-base text-purple-800">
					{{ currentItem.game_item_trap.game_bird.nickname }}
				</div>
			</div>
...
const useItem = async () => {
	// TODO: 实现使用陷阱逻辑
	message.info('使用陷阱功能待实现')
	// const res = await game.player_item_trap.use(currentItem.value.id, useCount.value)
	// if (res.code === 200) {
