<template>
	<div class="grid grid-cols-5 gap-4">
		...
		>
			...
			<div class="bg-blue-300 w-full aspect-square mb-2 flex items-center justify-center overflow-hidden">
				<img
					:src="getImageUrl('nest_buff', item.game_item_nest_buff?.nickname || 'default')"
					:alt="item.game_item_nest_buff?.nickname"
			...
			<div class="grow text-sm p-2 min-w-0">
				<p class="text-orange-500 font-bold truncate">
					{{ item.game_item_nest_buff?.nickname || '未知物品' }}
				</p>
				...
				<div class="text-xs text-purple-500 mt-1">
					时间减免: {{ item.game_item_nest_buff?.time || 0 }}%
				</div>
			</div>
		</el-card>
	</div>
	...
	<el-dialog v-model="dialogVisible" class="p-0!" width="90%" :show-close="false" header-class="p-0!">
		<el-image :src="getImageUrl('nest_buff', currentItem?.game_item_nest_buff?.nickname, '_big')" fit="cover" loading="lazy">
		...
		<div class="flex flex-col p-4! gap-2!">
			<div class="mb-3 text-base">{{ currentItem?.game_item_nest_buff?.nickname }}</div>
			<div class="text-gray-500 text-sm">{{ currentItem?.game_item_nest_buff?.desc }}</div>
			...
			<div class="mt-2">
				...
				<div class="bg-purple-50 p-2 rounded text-sm">
					巢穴时间减免: <span class="text-purple-600 font-bold">{{ currentItem?.game_item_nest_buff?.time || 0 }}%</span>
				</div>
			</div>
