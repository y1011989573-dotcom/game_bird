<template>
    <div class="grid grid-cols-5 gap-4">
        ...
        >
            <div class="bg-purple-300 w-full aspect-square mb-2 flex items-center justify-center overflow-hidden">
                <img
                    :src="getImageUrl('common', item.game_item_common?.nickname || 'default')"
                    :alt="item.game_item_common?.nickname"
            ...
            <div class="grow text-sm p-2 min-w-0">
                <p class="text-orange-500 font-bold truncate">
                    {{ item.game_item_common?.nickname || '未知物品' }}
                </p>
                <div class="text-xs text-gray-400 mt-1">
                    数量: {{ item.count }}
                </div>
                <div class="text-xs text-gray-500 mt-1 truncate">
                    {{ item.game_item_common?.desc || '暂无描述' }}
                </div>
            </div>
        </el-card>
    </div>
    ...
    <el-dialog v-model="dialogVisible" class="p-0!" width="90%" :show-close="false" header-class="p-0!">
        <el-image :src="getImageUrl('common', currentItem?.game_item_common?.nickname, '_big')" fit="cover" loading="lazy">
        ...
        <div class="flex flex-col p-4! gap-2!">
            <div class="mb-3 text-base">{{ currentItem?.game_item_common?.nickname }}</div>
            <div class="text-gray-500 text-sm">{{ currentItem?.game_item_common?.desc }}</div>
